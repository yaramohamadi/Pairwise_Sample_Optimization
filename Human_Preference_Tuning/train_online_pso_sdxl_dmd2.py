...
from pso_pytorch.diffusers_patch.sdxl_dmd_with_logprob import sdxl_dmd_pipeline_with_logprob
from pso_pytorch.diffusers_patch.distilled_inference_with_logprob import distilled_step_with_logprob
...
@torch.no_grad()
def log_validation_val_dataset(config, vae, unet, accelerator, weight_dtype, epoch, 
    ...
    for p_idx, prompt in enumerate(val_prompts):
        ...
        clip_scores.append(clip_score[0]) 
    ...
    for tracker in accelerator.trackers:
        ...
        tracker.log(
            {
                "validation": [
                    wandb.Image(image, caption=f"{i}: {val_prompts[i]}") 
        ...
        tracker.log({
            ...
            "CLIP Score on PickaPic test set": np.mean(clip_scores)}, commit=False)
...
def main(_):
    ...
    config.run_name = f'SDXL_DMD4_different_xT_{config.sample.num_steps}steps_sample_{config.train.distilled_train_steps}steps_train_lorarank{config.train.lora_rank}_lr{config.train.learning_rate}_beta{config.train.beta}_bspergpu{effective_batch_size_pergpu}'
    ...
    """
        make sure we remove the last time step for training
    """
    ...
    unet = UNet2DConditionModel.from_config(
                config.pretrained.pretrained_model_name_or_path, subfolder="unet")
    unet.load_state_dict(torch.load(
        hf_hub_download(dmd_repo, dmd_file, 
                        local_dir=config.general_cache_dir)
    ))
    ...
    # time ids
    def compute_time_ids(original_size=1024, crops_coords_top_left=0,):
    ...
    val_dataset = load_dataset(
        config.val_dataset,
        cache_dir=config.cache_val_dir,
    ...
    ## remove some timesteps in training if needed
    ...
    """
      For online SDXL-DMD, remove the last timestep in training, which is deterministic
    """
    global_step = 0
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            add_time_ids = torch.cat(
                    [compute_time_ids(1024, 0) for _ in range(bsz)]
            )
            ...
            images1, latents1, log_probs1 = sdxl_dmd_pipeline_with_logprob(
                accelerator,
                vae,
                unet,
                timesteps=distill_timesteps,
            ...
            images2, latents2, log_probs2 = sdxl_dmd_pipeline_with_logprob(
                accelerator,
                vae,
                unet,
                timesteps=distill_timesteps,
            ...
            log_probs = torch.stack([log_probs1, log_probs2], dim=1)  # (batch_size, 2, num_steps, 1)

            """
               remove the final timestep & latents
            """
            timesteps = distill_timesteps.unsqueeze(0).repeat(config.sample.batch_size, 1)[:, :-1]  # (batch_size, num_steps-1)
            latents = latents[:, :, :-1]
            ...
            images = torch.stack([images1, images2], dim=0).reshape(-1, 3, 1024, 1024) # (batch_size*2, C, H, W)
            ...
            rewards1 = ps_selector.score([Image.fromarray(img) for img in images1], train_prompts)
            rewards2 = ps_selector.score([Image.fromarray(img) for img in images2], train_prompts)
        ...
        # if epoch % 10 == 0:
        #     if accelerator.is_main_process:
        #         ## record the first 4 pairs
        #         images = ((samples["images"] + 1.0) * 127.5).clamp(0, 255).to(torch.uint8).permute(0, 2, 3, 1).cpu().numpy()[:8]
        #         accelerator.log(
        #             {
        #                 "Train Sampled Pair Images":[
        #                         wandb.Image(Image.fromarray(images[i]).resize((512, 512)),
        #                                     caption=f"reward: {rewards.reshape(-1)[i]:.6f}") 
        ...
        assert num_timesteps == num_train_timesteps, f"{num_timesteps}, {num_train_timesteps}"
            ...
            for i in range(0, total_batch_size, config.train.batch_size):
                ...
                for j in range(num_train_timesteps):
                    ...
                    with accelerator.accumulate(unet):
                        ...
                        # pdb.set_trace()
                        _, total_prob_0 = distilled_step_with_logprob(
                            noise_scheduler,
                            model_output=noise_pred_0,
                            timestep=sample_0["timesteps"][:, j],
                            prev_timestep=sample_0["timesteps"][:, j]-step_ratio,
                            sample=sample_0["latents"][:, j],
                            prev_sample=sample_0["next_latents"][:, j],
                        )
                        _, total_ref_prob_0 = distilled_step_with_logprob(
                            noise_scheduler,
                        ...
                        _, total_prob_1 = distilled_step_with_logprob(
                            noise_scheduler,
                            model_output=noise_pred_1,
                            timestep=sample_1["timesteps"][:, j],
                            prev_timestep=sample_1["timesteps"][:, j]-step_ratio,
                            sample=sample_1["latents"][:, j],
                            prev_sample=sample_1["next_latents"][:, j],
                        )
                        _, total_ref_prob_1 = distilled_step_with_logprob(
                            noise_scheduler,
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if accelerator.is_main_process:
                            if global_step % config.checkpointing_steps == 0: # or global_step == 1:
                                ...
                                log_validation_val_dataset(config, vae, accelerator.unwrap_model(unet),
                                                            accelerator, weight_dtype, global_step,
                                                            lora_dir=save_path, val_prompts=val_prompts,
                                                            pickscore_scorer=ps_selector, 
                                                            clip_scorer=clip_selector)
